...
# author: ortizimo
# date: 20250720-1249
# update: 20250926-1627
# version: 1.2
# desc: very basic script to remove unwanted words from rom filenames
# why: because there is no FileBot program to do this for roms so I made my own

# instructions:
#	1. Make a list of all the words you want to remove
#	2. Place this script in home/ in a Linux system or VM
#	3. Set a SHARED FOLDER path to the folder where ROMS are located
#	4. If the ROMS and in the same Linux system, then add the script to the same directory
#	5. Give the script execute rights (i.e. sudo chmod +x romnamer)
#	6. Run it (e.g., ./romnamer)
...
while true; do
    ...
    read -p "Do you want to generate a gamelist (Yy) or Exit now (Nn)? (y/n) " yn
